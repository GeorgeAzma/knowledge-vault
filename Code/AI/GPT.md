...
but [[Self Attention]] weights are fixed so relevance is constant
to have more versatility and allow more perspectives in what tokens are relevant 
multiple [[Self Attention]] heads are used
``` python
self_attention_blocks.forward(word_embedding[0, :])
```
3. [[Residual Connection]] for [[Self Attention]] output and (word embedding + positional encoding) output
