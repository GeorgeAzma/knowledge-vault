...
def is_injective(func):
    ...
    for value in func.values():
        if value in values_seen:
            return False  # Found a duplicate value, not injective
        values_seen.add(value)
    return True  # No duplicates found, injective

def is_surjective(func, codomain):
	# Check if all codomain elements are mapped
    return set(func.values()) == set(codomain)  

def func_type(func, codomain):
    injective = is_injective(func)
    surjective = is_surjective(func, codomain)
...
function = {1: 'a', 2: 'b'} 
codomain = ['a', 'b', 'c']
func_type(function, codomain) # Injective
 
function = {1: 'a', 2: 'b', 3: 'c'} 
codomain = ['a', 'b', 'c']
func_type(function, codomain) # Injective + Surjective = Bijective
			
function = {1: 'a', 2: 'b', 3: 'c', 4: 'c'} 
codomain = ['a', 'b', 'c']
func_type(function, codomain) # Surjective
```
