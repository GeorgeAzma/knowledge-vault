[[Cumulative Distribution Function (CDF)|CDF]] of [[Normal Distribution]] $e^{-x^2}$, scaled by $\frac{2}{\sqrt\pi}$
### Formula $\operatorname{erf}(x)=\frac{2}{\sqrt{\pi}}\int_0^xe^{-t^2}dt$
> [!tip] 
> - $\operatorname{erf}(\frac{\alpha-\mu}{\sigma\sqrt2})$ is probability that sample is in range $[-\alpha;\alpha]$
> - **Approximations**
