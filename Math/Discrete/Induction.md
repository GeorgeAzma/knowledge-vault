...
> $\dfrac{1(1+1)}{2}=1$ `holds true`
> **Inductive step** $n=k$ `to avoid confusion`
> assume inductive hypothesis holds for $k$, so $S_k=\dfrac{k(k+1)}{2}$ `S_k is short for sum`
> prove it for $k+1$, $S_{k+1}=S_k+(k+1)$
> substitute hypothesis $S_{k+1}=\dfrac{k(k+1)}{2}+(k+1)=\dfrac{k(k+1)+2(k+1)}{2}=\dfrac{(k+2)(k+1)}{2}$
> which is same as hypothesis formula for $n=k+1$ if you rearange
> $S_{k+1}=\dfrac{(k+1)((k+1)+1)}{2}$, if $n=k+1$ then $S_{k+1}=\dfrac{n(n+1)}{2}$
> 
