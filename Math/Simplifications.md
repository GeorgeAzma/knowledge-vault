...
### [[Trigonometry]]
- $\sin^2{x}+\cos^2{x}=1$ `Trigonometric identities`
- $\sinh^2{x}-\cosh^2{x}=1$ 
- $1+\tan^2{x}=\sec^2{x}$
- $1+\cot^2{x}=\csc^2{x}$
...
- $\tan a/2=\large\frac{\sin a}{1+\cos a}$
- $\sin(-x)=-\sin{x}$
- $\cos(-x)=\cos{x}$
...
> Only if $a,b$ are normalized, otherwise $|a||b|\cos\theta$ and $|a||b|\sin\theta$
> You can also think $dot$ as shadow of one vector to another
> And $cross$ as height from one vector to another
### [[Cross Product]]
