...
- $n$ sample count
> [!note] 
> - Here [[Null Hypothesis]] is that $\mu=\mu_0$
> - Using sample [[Mean]] $n-1$ instead of population mean $n$

> [!example] Testing that RNG mean is 0
> $r=[0.5,0.1,-0.3,0.9,-0.7]$ `-1 to 1` $\mu=0.1$ $\sigma\approx0.57$
> $t\approx\frac{0.1-0}{0.57/\sqrt{4}}\approx\frac{0.1}{0.29}\approx0.35$
> [[p-value]] for $0.35$ with $4$ [[Math/Degrees of Freedom (DOF)|Degrees of Freedom (DOF)]] is $74\%$
> which means there is $74\%$ chance that mean is actually $0$ 
...
- $v$ [[Degrees Of Freedom (DOF)]]
> [!tip] Use standard [[Normal Distribution]] for $v\geq30$
### Get [[p-value]] $2\int_{|t_{value}|}^\infty f(t,v)dt$
> [!note] Above is [[Cumulative Distribution Function (CDF)|CDF]] of t-distribution 
...
> [!tip] Use standard [[Normal Distribution]] [[Cumulative Distribution Function (CDF)|CDF]] for $v\geq30$
